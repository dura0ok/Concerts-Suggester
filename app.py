...
async def root(url: Optional[list[str]] = fastapi.Query(
    default=None,
    description='List of URLs to YandexMusic-playlists',
)):
    return await playlist_artists_controller.get_playlist_artists(playlists_urls=url)


@app_v1.get("/artist-concerts", responses=None)
async def root(id: list[str] = fastapi.Query(
    default=None,
    description='List of Ids of YandexMusic-artists'),
):
